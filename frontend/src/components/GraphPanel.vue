    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'Updating GraphRAG long/short-term memory in real-time' : 'Updating in real-time...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A few items are still processing; manual refresh in a few moments is recommended.</span>
...
const showSimulationFinishedHint = ref(false) // Hint shown after simulation finishes
const wasSimulating = ref(false) // Track if was simulating before
...
// Watch isSimulating changes to detect simulation finish
...
const renderGraph = () => {
  ...
  // Count edges between pairs of nodes, collect self-loop edges
  tempEdges.forEach(e => {
  ...
  // Record current edge index for each pair of nodes
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // This ensures all edges are distributed in the same reference frame and won't overlap due to different directions
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity towards center to group independent node clusters in center area
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to line direction, calculated based on distance ratio to ensure curve is clearly visible
    // The more edges, the larger the offset ratio relative to distance
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, increase by 5% for each additional edge
  ...
  // Save reference for external control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if dragging truly started (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation upon first detection of true drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Gradually stop simulation only if truly dragged
        if (d._isDragging) {
...
/* Hint style after simulation finishes */
