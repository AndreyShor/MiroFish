...
// Get maxRounds from query parameters during initialization to ensure child components get the value immediately
...
const handleGoBack = async () => {
  ...
  try {
    // Try gracefully closing the simulation environment first
  ...
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Failed to get time config, using default value: ${minutesPerRound.value} min/round`)
      }
...
const loadGraph = async (graphId) => {
  // When simulating, auto-refresh doesn't show full-screen loading to avoid flickering
...
onMounted(() => {
  ...
  // Record maxRounds configuration (value already obtained from query parameters during initialization)
  if (maxRounds.value) {
