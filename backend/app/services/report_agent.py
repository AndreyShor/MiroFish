...
3. Use ReACT multi-round thinking and reflection for each section
4. Support dialogue with users, autonomously calling retrieval tools within the conversation
"""
...
class ReportLogger:
    ...
    Generate agent_log.jsonl in the report folder to record detailed actions for each step.
    ...
    def log_llm_response(
        ...
        self.log(
            ...
            details={
                ...
                "message": f"LLM response (Tool core: {has_tool_calls}, Final answer: {has_final_answer})"
            }
        )
    ...
    def log_section_content(
        ...
        """Record section content generation complete (records content only, does not mean the whole section is finished)"""
        self.log(
...
class ReportConsoleLogger:
    ...
    These logs are different from agent_log.jsonl and are in plain text console output format.
    """
    ...
    def _setup_file_handler(self):
        """Set file handler to write logs to file as well"""
        import logging
...
[Return Content]
- Relevant original facts (can be directly cited)
- Core entity insights
- Relationship chain analysis"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Wide Search - Get Full Panorama View]
This tool is used to obtain the full panorama of simulation results, especially suitable for understanding the evolution process of an event. It will:
...
This is not LLM simulation, but a call to real interview interfaces to get raw answers from simulation Agents.
By default, interview on both Twitter and Reddit platforms simultaneously to get more comprehensive views.
...
[Usage Scenarios]
- Need to understand opinions on events from different role perspectives (What do students think? What does the media think? What does the official say?)
...
[Important] Required OASIS simulation environment to be running to use this feature!"""

# -- Outline Planning Prompt --

PLAN_SYSTEM_PROMPT = """\
You are an expert at writing "Future Prediction Reports" with a "God's View" of the simulation world—you can perceive the behavior, remarks, and interactions of every Agent in the simulation.

[Core Concept]
We have built a simulation world and injected specific "simulation requirements" as variables. The evolution results of the simulation world are predictions of what might happen in the future. You are not observing "experimental data" but a "rehearsal for the future".
...
[Report Positioning]
- ✅ This is a simulation-based future prediction report, revealing "what if this happens, what will the future be like"
- ✅ Focus on prediction results: event progression, group reactions, emergent phenomena, potential risks
- ✅ Agent actions and words in the simulation world are predictions of future population behavior
- ❌ Not an analysis of the current state of the real world
- ❌ Not a general summary of public opinion

[Section Count Limits]
- Minimum 2 sections, maximum 5 sections
- No sub-sections required; write complete content for each section directly
...
[Some Sample Future Facts Predicted by Simulation]
{related_facts_json}

Please examine this future rehearsal with a "God's View":
...
You are an expert at writing "Future Prediction Reports" and are currently writing a section of the report.

Report Title: {report_title}
Report Summary: {report_summary}
Prediction Scenario (Simulation Requirement): {simulation_requirement}

Current section to be written: {section_title}

═══════════════════════════════════════════════════════════════
[Core Concept]
═══════════════════════════════════════════════════════════════

The simulation world is a rehearsal for the future. We injected specific conditions into the simulation world (simulation requirement),
and the behavior and interaction of Agents in the simulation are predictions of future population behavior.

Your task is to:
- Reveal what happened in the future under the set conditions
- Predict how various types of people (Agents) reacted and acted
...
1. [Must call tools to observe the simulation world]
   - You are observing the future rehearsal with a "God's View"
   - All content must come from events that occurred in the simulation world and Agent words/actions
...
2. [Must quote original Agent words and actions]
   ...
   - Use quote format in reports to show these predictions, for example:
...
3. [Language Consistency - Quoted content must be translated into the report language]
   - Tool return content may contain English or mixed Chinese-English expressions
   - If the simulation requirements and original materials are Chinese, the report must be written entirely in Chinese
   - When you quote English or mixed content returned by tools, you must translate it into fluent Chinese before writing it into the report
   - Maintain the original meaning unchanged when translating, ensuring the expression is natural and smooth
   - This rule applies to both the body text and content in blockquote (> format)

4. [Faithfully present prediction results]
   - Report content must reflect the simulation results that represent the future in the simulation world
   - Do not add information that does not exist in the simulation
   - If information in some aspect is insufficient, state so truthfully
...
- ❌ Forbidden to use any Markdown headings (#, ##, ###, ####, etc.) within a section
- ❌ Forbidden to add the main section title at the beginning of the content
- ✅ Section titles are automatically added by the system; you only need to write the body content
...
Platform X, as the first scene of public opinion, assumed the core function of initial information release:

> "Platform X contributed 68% of the initial volume..."

**Emotion Amplification Stage**

Platform Y further amplified the influence of the event:
...
#### 1.1 Detailed Analysis   ← Error! Do not use #### for fine segmentation
...
- insight_forge: Deep insight analysis, automatically decomposing questions and searching for facts and relationships across multiple dimensions
- panorama_search: Wide-angle panorama search, understanding event overview, timeline, and evolution process
- quick_search: Fast verification of a specific information point
- interview_agents: Interview simulation Agents, obtaining first-person perspectives and real reactions from different roles
...
The system will execute the tool and return the result to you. You do not need and cannot write the tool return results yourself.
...
2. Cite a large number of original texts to show simulation effects
3. Use Markdown format (but forbid using headings):
   - Use **bold text** to mark key points (replaces sub-headings)
   - Use lists (- or 1.2.3.) to organize key points
   - Use blank lines to separate different paragraphs
   - ❌ Forbidden to use any heading syntax such as #, ##, ###, ####, etc.
4. [Quote Format Specification - Must be a separate paragraph]
   ...
   ✅ Correct format:
   ```
   The response from the school was considered lacking in substance.
   ...
   ❌ Error format:
   ```
   The response from the school was considered lacking in substance. > "The school's response pattern..." This evaluation reflected...
   ```
5. Maintain logical consistency with other sections
6. [Avoid Duplication] Read the completed section content below carefully and do not duplicate the same info
7. [Again Emphasize] Do not add any headings! Replace sub-section headings with **bold** text"""
...
- ✅ Section headings are added by the system automatically
- ✅ Directly write the body, using **bold** to replace sub-section headings
...
═══════════════════════════════════════════════════════════════
Called tool {tool_calls_count}/{max_tool_calls} times (Used: {used_tools_str}){unused_hint}
- If information is sufficient: Start with "Final Answer:" to output section content (must cite original texts above)
...
REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used: {unused_list} yet. It's recommended to try different tools to get multi-perspective information."
...
[Background]
Prediction Requirement: {simulation_requirement}
...
2. Answer questions directly, avoiding lengthy reasoning and discourse
...
[Available Tools] (Only use when necessary, at most 1-2 calls)
{tools_description}
...
[Answer Style]
- Concise and direct, no long-winded paragraphs
...
class ReportAgent:
        ...
        return {
            "insight_forge": {
                ...
                "parameters": {
                    ...
                    "report_context": "Current report section context (optional, helps generate more accurate sub-questions)"
                }
            },
            "panorama_search": {
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM outputs bare JSON (not wrapped in <tool_call> tags)
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflict count where both tool call and Final Answer appear
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Final iteration also returns None, break loop to force finish
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM output both tool call and Final Answer ({conflict_retries} conflict)"
                )
                ...
                if conflict_retries <= 2:
                    ...
                    # Third time: Graceful degradation, truncate to the first tool call and force execution
                    logger.warning(
            ...
            # -- Case 2: LLM attempt to call tool --
            if has_tool_calls:
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM attempted to call {len(tool_calls)} tools, executing only the first one: {call['name']}")
        ...
        # Check if LLM return is None during forced closing
        if response is None:
            logger.error(f"Section {section.title} LLM returned None during forced closing, using default error message")
    ...
    def generate_report(
        ...
        """
        Generate full report (section-by-section real-time output)
        
        Each section is saved to the folder immediately after completion, no need to wait for the whole report.
        File structure:
        reports/{report_id}/
        ...
        # If report_id is not passed, generate it automatically
        if not report_id:
        ...
        try:
            ...
            # Stage 2: Section-by-section generation (save by section)
            ...
            try:
                ...
                pass  # Ignore errors from failed saving
    ...
    def chat(
        ...
        In the chat, the Agent can independently call retrieval tools to answer questions
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Line number to start reading from (for incremental retrieval, 0 from start)
        ...
        return {
            ...
            "from_line": from_line,
            "has_more": False  # Already reached the end
        }
    
    @classmethod
    def get_console_log_stream(cls, report_id: str) -> List[str]:
        """
        Get full console log (get all at once)
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get full Agent log (used to get all at once)
    ...
    def save_outline(cls, report_id: str, outline: ReportOutline) -> None:
        ...
        Called immediately after the planning stage is complete
        """
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings of the section title (skip duplicates within first 5 lines)
                if i < 5:
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble full report from saved section files and perform heading cleanup
        """
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        3. Clean redundant empty lines and horizontal rules
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings (headings with same content appearing within 5 consecutive lines)
                ...
                # - ### and below (level>=3) Convert to bold text
                    ...
                    if title == outline.title:
                        ...
                        # Section title used # by mistake, corrected to ##
