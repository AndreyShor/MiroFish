...
Dynamically update Agent activity in simulation to Zep graph
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        Use natural language description format to enable Zep to extract entities and relationships
    ...
    def _describe_repost(self) -> str:
        """Repost post - includes original content and author info"""
    ...
    def _describe_quote_post(self) -> str:
        """Quoted post - includes original content, author info, and quote comment"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_follow(self) -> str:
        """Followed user - includes the target user's name"""
    ...
    def _describe_create_comment(self) -> str:
        """Posted comment - includes comment content and post info"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_like_comment(self) -> str:
        """Liked comment - includes comment content and author info"""
    ...
    def _describe_dislike_comment(self) -> str:
        """Disliked comment - includes comment content and author info"""
        comment_content = self.action_args.get("comment_content", "")
        comment_author = self.action_args.get("comment_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Monitor simulated actions log file and update new agent activities to Zep graph in real-time.
    ...
    - Original post content of likes/dislikes
    - Original post content of reposts/quotes
    - Usernames of follows/mutes
    - Original comment content of likes/dislikes
    """
    
    # Batch send size (send after accumulating this many activities per platform)
    BATCH_SIZE = 5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Activity buffers grouped by platform (each per-platform buffer sends after reaching BATCH_SIZE)
        ...
        self._total_items_sent = 0  # Total items successfully sent to Zep
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will contain complete context information (e.g., original post, username).
    ...
    def _worker_loop(self):
        """Background worker loop - send activities to Zep in batches by platform"""
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Fail to send batch to Zep (Attempt {attempt + 1}/{self.MAX_RETRIES}): {e}")
    ...
    def _flush_remaining(self):
        ...
        # First handle remaining activities in queue, add to buffers
...
class ZepGraphMemoryManager:
    ...
    def get_all_stats(cls) -> Dict[str, Dict[str, Any]]:
        """Get statistical information for all updaters"""
        return {
