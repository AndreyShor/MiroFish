...
# Adding this prefix can avoid Agent calling tools and reply directly with text
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview prompt, add prefix to avoid Agent calling tools
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match predefined entity types (Labels are not just Entity nodes)
...
def create_simulation():
    ...
    Note: Parameters like max_rounds are automatically generated by LLM, no need to set manually
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Running scripts (run_*.py) stay in backend/scripts/ directory, no longer copied to simulation directory
    ...
    try:
        ...
        # These statuses all mean preparation is complete:
        # - ready: Ready, can run
        # - preparing: If config_generated=True, means complete
        # - running: Running, means preparation was done long ago
        # - completed: Completed, means preparation was done long ago
...
def prepare_simulation():
    ...
    Features:
    - Automatically detect completed preparation work to avoid repeated generation
    ...
    Request (JSON):
        {
            ...
            "parallel_profile_count": 5,                  // Optional, number of personas generated in parallel, default 5
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Already have completed preparation work",
    ...
    try:
        ...
        # Check if already prepared (avoid repeated generation)
        if not force_regenerate:
            ...
            if is_prepared:
                logger.info(f"Simulation {simulation_id} already prepared, skipping repeated generation")
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
                        "status": "ready",
                        "message": "Already have completed preparation work, no need to regenerate",
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # No edge info needed, speed up
            )
        ...
        def run_prepare():
            try:
                ...
                # Update simulation status as failed
                state = manager.get_simulation(simulation_id)
                if state:
...
def get_prepare_status():
    ...
    2. Check if already prepared preparation work via simulation_id
    ...
    try:
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Already have completed preparation work",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task_id:
            if simulation_id:
                # simulation_id provided but not prepared
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # Task does not exist, but if simulation_id is provided, check if already prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    return the latest one if there are multiple (sorted by created_at)
    ...
    try:
        ...
        # Sort by creation time in descending order, return the latest one
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list with rich information like project name and description
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get running status (read user-set actual rounds from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use user-set total_rounds, or recommended rounds if none
            ...
            # Get file list of associated project (up to 3)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Reads file directly, does not pass through SimulationManager
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "platform": "reddit",
                "count": 15,
                "total_expected": 93,  // Total expected (if any)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g., file modification time, whether generating, etc.)
    - Even if configuration generation is not complete, part of the info can be returned
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - agent_configs: Activity configuration for each Agent (activity, speaking frequency, stance, etc.)
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script file (general script, located in backend/scripts/)
...
# ============== Profile Generation Interfaces (used independently) ==============
...
# ============== Simulation Run Control Interfaces ==============
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, it will first stop and clean run logs
        ...
        - Suitable for scenarios where direct rerun of simulation is needed

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) in simulation will be updated to Zep graph in real-time
        - This allows the graph to "remember" the simulation process for subsequent analysis or AI dialogue
        - Requires the project associated with simulation to have a valid graph_id
    ...
    try:
        ...
        # Smarter status handling: If preparation complete, allow restart
        if state.status != SimulationStatus.READY:
            # Check if preparation work is complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation work complete, check if there are running processes
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is running, please call /stop interface to stop first, or use force=true to force restart"
                            }), 400
...
# ============== Real-time Status Monitoring Interfaces ==============

@simulation_bp.route('/<simulation_id>/run-status', methods=['GET'])
def get_run_status(simulation_id: str):
    """
    Get real-time simulation running status (for frontend polling)
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed status of simulation run (including all actions)
    ...
    try:
        ...
        # recent_actions only shows contents of both platforms for the latest round
...
def get_simulation_actions(simulation_id: str):
    """
    Get history of Agent actions in simulation
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (waiting for commands after completing the simulation loop)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // Optional, specify platform (twitter/reddit)
                                               // If not specified: interviewed on both platforms simultaneously in dual-platform simulation
            "timeout": 60                      // Optional, timeout in seconds, default 60
        }

    Return (without specifying platform, dual-platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or closed. Please ensure simulation is complete and in waiting-for-command mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think of B?"  // Default value used if platform not specified
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by each item's platform)
                                               // If not specified: interviewed on both platforms simultaneously for each Agent in dual-platform simulation
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"The platform of interview item {i+1} can only be 'twitter' or 'reddit'"
                }), 400
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What's your general opinion on this?",  // Required, interview question (same question for all Agents)
...
def close_simulation_env():
    ...
    Send a command to close the environment to the simulation, allowing it to gracefully exit the waiting-for-command mode.
    
    Note: This is different from the /stop interface, which forcefully terminates the process,
    whereas this interface lets the simulation gracefully shut down the environment and exit.
