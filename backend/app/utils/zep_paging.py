"""Zep Graph pagination retrieval tool.

Zep's node/edge list API uses UUID cursors for pagination.
This module encapsulates automatic pagination logic (including single-page retry),
returning the full list transparently to the caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request, exponential backoff on failure. Only retries network/IO transient errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Get graph nodes with pagination, returns at most max_items (default 2000). Includes retries for each page request."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Get all graph edges with pagination, returns the full list. Includes retries for each page request."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
