...
Uses project context mechanism, persisting state on the server
"""
...
def build_graph():
    """
    Interface 2: Build graph according to project_id
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is building, please do not resubmit. To force rebuild, please add force: true",
        ...
        def build_task():
            ...
            try:
                ...
                task_manager.update_task(
                    task_id,
                    message=f"Starting addition of {total_chunks} text chunks...",
                    progress=15
                )
                ...
                # Wait for Zep processing completion (query each episode's processed status)
                task_manager.update_task(
                    task_id,
