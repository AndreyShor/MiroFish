...
ONTOLOGY_SYSTEM_PROMPT = """You are an expert in knowledge graph ontology design. Your task is to analyze the given text content and simulation requirements to design entity types and relationship types suitable for **social media public opinion simulation**.
...
A. **Fallback types (must be included, placed at the last 2 of the list)**:
   - `Person`: Fallback type for any individual. Used when a person does not belong to other more specific personal types.
   - `Organization`: Fallback type for any organization. Used when an organization does not belong to other more specific organization types.
...
B. **Specific types (8 types, designed based on text content)**:
   ...
   - Example: If the text relates to academic events, you can have `Student`, `Professor`, `University`.
   - Example: If the text relates to business events, you can have `Company`, `CEO`, `Employee`.

**Why fallback types are needed**:
- Various people appear in the text, such as "primary and secondary school teachers", "passerby A", "a netizen".
...
- Ensure that the source_targets of the relationship cover the entity types you define.
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # Truncate if text exceeds 50,000 characters (only affects LLM content, not graph construction)
...
3. The first 8 are specific types designed according to the text content
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fallbacks_to_add:
            ...
            # If adding fallbacks exceeds 10, remove some existing types
            if current_count + needed_slots > MAX_ENTITY_TYPES:
                # Calculate how many to remove
                to_remove = current_count + needed_slots - MAX_ENTITY_TYPES
                # Remove from the end (keep more important specific types at the beginning)
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'Automatically generated by MiroFish for social simulation',
            '"""',
            '',
