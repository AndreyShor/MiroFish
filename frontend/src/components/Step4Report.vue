...
const toolConfig = {
  ...
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Bolt icon - represents quickness
  },
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-(?:\s*Select|Select)([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        if (name) {
          ...
          // Continuation of reason (exclude summary paragraphs at the end)
    ...
    const bioMatch = block.match(/_(?:Bio)[:：]\s*([\s\S]*?)_\n/)
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question has "1." prefix, special handling needed
    ...
    const answerMatch = block.match(/\*\*A[:：]\*\*\s*([\s\S]*?)(?=\*\*(?:Key Quotes)|$)/)
      if (answerMatch) {
        ...
        // Platform fallback logic (compat with old format: only one platform tag)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Forecast Scenario: '),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts from chronological memory'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No entities involved available'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const cleanQuoteText = (text) => {
      if (!text) return ''
      // Remove leading patterns like "1. ", "2. ", "1. ", "(1)" etc.
      return text.replace(/^\s*\d+[\.\、\)）]\s*/, '').trim()
    }
    
    const activeIndex = ref(0)
    const expandedAnswers = ref(new Set())
    // Maintain independent platform choice for each Q&A pair
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Get current platform choice for a specific question
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Set platform choice for a specific question
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (English format, backend new format)
    ...
    // Get answer corresponding to specific question
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split successfully and index valid
    ...
    // Check if question has dual-platform answers (filtered placeholders)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholder) and contents differ
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch (only show if real dual-platform answers exist)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relevant Relations'),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relevant Nodes'),
...
const activeStep = computed(() => {
  ...
  // If none active, return the last completed step
...
const renderMarkdown = (content) => {
  ...
  // Remove the leading H2 title (## xxx), as the section title is already displayed in the outer layer
  ...
  // Handle line breaks - double empty lines become paragraph separators, single line break becomes <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean instances where <p><br> is immediately followed by a block-level element (due to extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> immediately following a block-level element and before a paragraph start tag
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single item <ol> is separated by paragraph content, keep the numbering increasing
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scrolling logic handled unified in nextTick after loop
          }
...
const extractFinalContent = (response) => {
  ...
  // If starting with ##, # or >, it might be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and contains markdown, try to return after removing thought process
