...
const handleNextStep = (params = {}) => {
  ...
  // If there are custom rounds, pass them through query parameters
  if (params.maxRounds) {
 ...
 * When user returns from Step 3 to Step 2, default to exiting the simulation
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Try gracefully closing the simulation environment
      try {
        ...
        // If graceful close exception, try forced stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Failcheck for simulation state checking
