...
if sys.platform == 'win32':
    ...
    # Force default encoding (affects default encoding of open() function)
    # Note: This needs to be set at Python startup; setting it at runtime might not take effect
...
# Add filter immediately upon module loading to ensure it takes effect before camel code executes
...
def disable_oasis_logging():
    ...
    OASIS logging is too redundant (records every agent's observations and actions), we use our own action_logger
    """
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if success_count > 0:
            ...
            print(f"  Interview completed: agent_id={agent_id}, success platform count={success_count}/{len(platforms_to_interview)}")
            return True
        else:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Those needing to interview both platforms
        ...
        for interview in interviews:
            ...
            if item_platform == "twitter":
                ...
                # Platform unspecified: interview both
        ...
        # Handle interviews for Twitter platform
        ...
        # Handle interviews for Reddit platform
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of "Agent_0" style IDs
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last read max rowid (use rowid instead of created_at because created_at formats differ between platforms)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keeping only key fields (keep full content, do not truncate)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: supplement original post content and author
            ...
            if new_post_id:
                # original_post_id for repost points to the original post
                cursor.execute("""
        ...
        # Quote post: supplement original post content, author, and quote comment
            ...
            # Get quote content from the quoted post
            if new_post_id:
                cursor.execute("""
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, different platforms can use different API providers during parallel simulation to increase concurrency capacity.
    ...
    # Use config as backup if no model name in .env
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # Twitter uses common LLM configuration
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, otherwise falls back to common configuration)
    ...
    # Get Agent real name mapping from configuration (use entity_name instead of default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # If an agent is not in the configuration, use OASIS default name
    for agent_id, agent in result.agent_graph.get_agents():
        if agent_id not in agent_names:
            agent_names[agent_id] = getattr(agent, 'name', f'Agent_{agent_id}')
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation, do not enter wait for commands mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at start of main to ensure whole program responds to exit signal
    ...
    log_manager.info(f"  - Total duration: {total_hours} hours")
    log_manager.info(f"  - Round duration: {minutes_per_round} minutes")
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering wait for commands mode - environment remains running")
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure correct exit upon receiving SIGTERM/SIGINT
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (allow resource cleanup)
            if _shutdown_event:
                _shutdown_event.set()
        
        # Do not sys.exit() directly, let asyncio loop exit naturally and clean up
        # If signal is received again, then force exit
        else:
            print("Forcing exit...")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up multiprocessing resource tracker (prevent warnings at exit)
        try:
