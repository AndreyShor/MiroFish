      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Display hint if there are more files -->
        ...
        <!-- Card Title (uses first 20 characters of simulation requirements) -->
        ...
        <!-- Bottom Decor Line (expands on hover) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Start Simulation" and Step 5 "Deep Interaction" must be started in real-time and do not support historical playback.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Expanded state: calculate height based on number of cards
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate number of cards in current row to center each row
    ...
    const x = offset * 35
    // Adjust start position, close to title but maintaining proper spacing
...
// Get progress class based on round progress
...
// From simulation requirement generate title (take first 20 chars)
...
// Format simulation_id for display (take first 6 characters)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, just record state and handle after animation finish
        if (isAnimating) return
        
        // If target state is same as current, no need to handle
        ...
        // Short delay for expand (50ms), longer for fold (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Unlock after animation finish, check for pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // Check for new pending state after animation finish
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay for a short time before executing to avoid rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, contract from bottom of viewport, need more scroll to trigger expand
...
onMounted(async () => {
  // Ensure data load completion after DOM render
  await nextTick()
  await loadHistory()
...
/* Create fixed spacing square grid with CSS background patterns */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from top-left, only extend bottom when height changes, without affecting existing grid positions */
...
.cards-container {
  ...
  /* min-height is dynamically calculated by JS based on number of cards */
}
