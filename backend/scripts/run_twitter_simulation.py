...
This script reads parameters from the configuration file to execute the simulation, achieving full automation

Features:
- Does not close environment immediately after finishing simulation, enters wait for commands mode
...
Usage:
    ...
    python run_twitter_simulation.py --config /path/to/simulation_config.json --no-wait  # Close immediately after finishing
"""
...
# Load .env file from project root (containing LLM_API_KEY etc.)
...
class UnicodeFormatter(logging.Formatter):
    """Custom formatter, converts Unicode escape sequences to readable characters"""
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: Whether to wait for commands after completing simulation (default True)
        """
    ...
    def _create_model(self):
        ...
        Uses project root .env configuration:
        - LLM_API_KEY: API key
        ...
        # Use config as backup if not in .env
        if not llm_model:
            llm_model = self.config.get("llm_model", "gpt-4o-mini")
        
        # Set environment variables for camel-ai
        if llm_api_key:
            os.environ["OPENAI_API_KEY"] = llm_api_key
        
        if not os.environ.get("OPENAI_API_KEY"):
            raise ValueError("Missing API key. Please set LLM_API_KEY in project root .env")
    ...
    async def run(self, max_rounds: int = None):
        ...
        self.env = oasis.make(
            ...
            semaphore=30,  # Limit max concurrent LLM requests, prevent API overload
        )
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering wait for commands mode - Environment remains running")
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after finishing, do not enter wait for commands mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at start of main
    ...
    # Initialize log configuration (fixed filename, clean old logs)
...
def setup_signal_handlers():
    ...
    Allows program a chance to clean up resources (close DB, environment, etc.)
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force exit only if signal received multiple times
            print("Force exiting...")
            sys.exit(1)
