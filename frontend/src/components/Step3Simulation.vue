    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting...' : 'Generating Result Report' }} 
...
const doStartSimulation = async () => {
  ...
  // Reset all states first to ensure no influence from previous simulation
  resetAllState()
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Detected all platforms simulation ended')
        }
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Use all_actions to get full list of actions
      ...
      // Don't auto-scroll, allow user to browse timeline freely
      // New actions will be appended to the bottom
    }
  } catch (err) {
