...
Runs simulations in the background and records Agent actions, supporting real-time status monitoring.
"""
...
# Flag to indicate if cleanup function is registered
...
class SimulationRunState:
    ...
    # Platform completion status (detected by simulation_end event in actions.jsonl)
...
class SimulationRunner:
    ...
    2. Parsing run logs, recording actions of each Agent
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max rounds (optional, for truncating simulations)
        enable_graph_memory_update: bool = False,  # Whether to update Zep graph
        ...
        Args:
            ...
            max_rounds: Max rounds (optional, for truncating simulations)
        ...
        # Determine which script to run (script located in backend/scripts/ directory)
        if platform == "twitter":
        ...
        try:
            ...
            # Create main log file, avoid stdout/stderr pipe buffer full causing process block
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set child process environment variables, ensure UTF-8 encoding on Windows
            # This can fix encoding issues in third-party libraries (e.g. OASIS)
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support, let all open() default to UTF-8
            ...
            # Use start_new_session=True to create new process group, ensure all child processes can be terminated
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Final log read after process finished
            if os.path.exists(twitter_actions_log):
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ...
                    logger.info(f"Stop graph memory update: simulation_id={simulation_id}")
                except Exception as e:
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform running, just check that one
                                    # If two platforms running, both need to complete
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Overall round takes maximum of two platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time takes maximum of two platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Judge if platform is enabled by checking if corresponding actions.jsonl file exists
        ...
        # Check which platforms enabled (by file existence)
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = forced termination, /T = terminate process tree (including children)
            ...
            try:
                ...
                try:
                    ...
                    # Forced termination
                    logger.warning(f"Process not responding, forced termination: {simulation_id}")
                    subprocess.run(
            ...
            # As start_new_session=True is used, PGID equals main process PID
            ...
            try:
                ...
                # Forced SIGKILL if timeout
                logger.warning(f"Process group not responding to SIGTERM, forced termination: {simulation_id}")
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: priority to record's platform, then default
    ...
    def get_all_actions(
        ...
        Returns:
            Complete action list (sorted by timestamp, descending)
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
        
        # Read Twitter action file (set platform to twitter automatically based on path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (set platform to reddit automatically based on path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If separate platform files don't exist, try reading old single file format
        if not actions:
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Cleanup simulation run logs (for forcing simulation restart)
        ...
        - twitter_simulation.db (database)
        - reddit_simulation.db (database)
        ...
        # List of files to delete (including databases)
        files_to_delete = [
            "run_state.json",
            "simulation.log",
            "stdout.log",
        ...
        # Cleanup action logs in platform directories
    ...
    def cleanup_all_simulations(cls):
        ...
        # Stop all graph memory updaters first (stop_all will print logs internally)
        try:
        ...
        for simulation_id, process in processes:
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        # Use cross-platform termination method
                        cls._terminate_process(process, simulation_id, timeout=5)
                    except (ProcessLookupError, OSError):
                        # Process might already be non-existent, try direct termination
                        try:
                            process.terminate()
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts to ensure all simulation processes are cleaned when server shuts down
        """
        ...
        # If not in debug mode, there is no such environment variable, we still need to register
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark registered to prevent child process from attempting again
            return
        
        # Save original signal handlers
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP only exists in Unix systems (macOS/Linux)
        ...
        def cleanup_handler(signum=None, frame=None):
            """Signal handler: cleanup simulation processes first, then call original handler"""
        ...
        try:
            ...
            # Not in main thread, only atexit can be used
            logger.warning("Failed to register signal handlers (not in main thread), only using atexit")
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: interview both platforms and consolidate results
    ...
    def close_simulation_env(
        ...
        """
        Close simulation environment (rather than stopping simulation process)
    ...
    def get_interview_history(
        ...
        Args:
            ...
            limit: Return count limit for each platform
        ...
        if platform in ("reddit", "twitter"):
            ...
            # Query both platforms when platform not specified
