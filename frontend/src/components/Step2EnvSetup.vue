    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combining context, automatically call tools to organize entities and relationships from the knowledge graph, initialize simulation individuals, and give them unique behaviors and memories based on reality seeds.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Topics Related to Reality Seeds</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets parameters such as world time flow rate, recommendation algorithms, active time periods for each individual, posting frequency, and event triggers based on simulation requirements and reality seeds.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation Rounds Configuration - shown only after config generation is complete and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish automatically plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of reality, with each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time passing.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>For Agent scale of 100: Estimated time around {{ Math.round(customMaxRounds * 0.6) }} mins</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        For Agent scale of 100: Estimated time {{ Math.round(autoGeneratedRounds * 0.6) }} mins
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If this is the first run, it is highly recommended to switch to "Custom Mode" and reduce the number of simulation rounds for a quick preview and lower risk of errors. ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Persona Age</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} years</span>
            </div>
            <div class="info-item">
              <span class="info-label">Persona Gender</span>
            ...
            <div class="info-item">
              <span class="info-label">Persona MBTI</span>
          ...
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">Topics Related to Reality Seeds</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Summary of experience and behavioral patterns</span>
              </div>
              <div class="dimension-card">
...
// State
const phase = ref(0) // 0: Init, 1: Generating Personas, 2: Generating Config, 3: Completed
...
// Log deduplication: record key information of last output
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation rounds configuration
const useCustomRounds = ref(false) // Default to automatic rounds
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generating Agent Personas' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Entering configuration generation phase, start polling configuration
    if (!configTimer) {
      addLog('Starting generation of dual-platform simulation configurations...')
      startConfigPolling()
    }
...
// Calculate automatically generated rounds from configuration (do not use hardcoded default values)
...
// Calculate total count of associated topics for all personas
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // User chose automated rounds, do not pass max_rounds parameter
    addLog(`Starting simulation, using auto-generated rounds: ${autoGeneratedRounds.value}`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected existing preparation work, using it directly.')
      ...
      // Set expected total agents immediately (from prepare API return)
      ...
      startPolling()
      // Start real-time Profile acquisition
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update when API returns valid values to avoid overwriting existing valid ones
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog('Generating Agent persona configurations...')
      ...
      if (data.config_generated && data.config) {
        ...
        addLog('✓ Simulation configuration completed')
        
        // Show detailed configuration summary
        if (data.summary) {
          addLog(`  ├─ Agent Count: ${data.summary.total_agents} items`)
          ...
          addLog(`  ├─ Hot Topics: ${data.summary.hot_topics_count} items`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time Config: ${tc.minutes_per_round} mins per round, ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} rounds in total`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup completed, simulation can start')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Show detailed configuration summary
        if (res.data.summary) {
          addLog(`  ├─ Agent Count: ${res.data.summary.total_agents} items`)
        ...
        addLog('Configuration generation in progress, starting to poll...')
        startConfigPolling()
      }
    }
  } catch (err) {
...
onMounted(() => {
  ...
  if (props.simulationId) {
    addLog('Step 2 Environment Setup Initialization')
